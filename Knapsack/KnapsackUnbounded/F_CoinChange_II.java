...
public class F_CoinChange_II {
    ...
    public static int change(int sum, int[] coins) {
        int n = coins.length;
        int INF = Integer.MAX_VALUE - 1; // to prevent overflow when adding 1

        int[][] dp = new int[n + 1][sum + 1];

        // Base Case Initialization
        for (int j = 0; j <= sum; j++)
            dp[0][j] = INF;  // can't make positive sum with 0 coins
        dp[0][0] = 0;
    ...
    public static void main(String[] args) {
        ...
        int  ans4 = change(sum, coins);
        System.out.println("Tabulation: " + ans4);
